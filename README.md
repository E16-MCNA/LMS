...
Hệ thống E16 LMS sử dụng cơ chế kiểm soát truy cập dựa trên vai trò (RBAC - Role-Based Access Control) chặt chẽ. Để tối ưu hóa và đơn giản hóa việc phân quyền trong cơ sở dữ liệu, **8 vai trò nghiệp vụ** được chuẩn hóa (normalized) thành **6 vai trò hệ thống chính** như sau:
...
* **Nghiệp vụ Tài chính (Finance)**: Thiết lập biểu phí học phần, đối soát và phê duyệt các giao dịch đóng học phí của sinh viên, theo dõi công nợ, tính lương tự động cho giảng viên dựa trên số tiết dạy và sĩ số.

...
export function registerEventHandlers() {
  ...
  eventBus.on("attendance.session.saved", async ({ courseId, records }, pool) => {
    for (const { studentId } of records || []) {
      ...
      const warningMessage = `Tỷ lệ chuyên cần dưới 80% (${attendancePct}%).`;
      ...
      if (attendancePct < 80) {
        ...
        if (!existingWarning) {
          await notifyStudent(pool, studentId, `Cảnh báo chuyên cần: Tỷ lệ đi học dưới 80% (${attendancePct}%).`, { relatedEntityType: "course", relatedEntityId: courseId });
          await notifyAdvisorOf(pool, studentId, `Học sinh được phân công có tỷ lệ chuyên cần dưới 80% (${attendancePct}%).`, { relatedEntityType: "course", relatedEntityId: courseId });
          await notifyParentOf(pool, studentId, `Cảnh báo chuyên cần của con em: Tỷ lệ đi học dưới 80% (${attendancePct}%).`, { relatedEntityType: "course", relatedEntityId: courseId });
        }
      }
      if (attendancePct < 60) {
        await pool.query(

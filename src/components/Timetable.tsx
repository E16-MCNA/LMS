...
export default function Timetable({ role, currentUser, store, onRefreshData, defaultLookupType, onRedirectToAttendance }: TimetableProps) {
  ...
  // Admin Lookup States
  const [lookupType, setLookupType] = useState<"student" | "teacher">(defaultLookupType === "teacher" ? "teacher" : "student");
  ...
  const [viewingSectionDetail, setViewingSectionDetail] = useState<CourseSection | null>(null);
  ...
  const [formStatus, setFormStatus] = useState<"pending" | "open" | "closed" | "cancelled">("open");
  ...
  const getSemesterAnchorDate = (semesterId: string) => {
    const semester = (store.semesters || []).find(s => s.id === semesterId);
    const today = new Date();
    const start = semester?.startDate ? new Date(String(semester.startDate).slice(0, 10)) : today;
    const end = semester?.endDate ? new Date(String(semester.endDate).slice(0, 10)) : null;

    if (Number.isNaN(start.getTime())) return today;
    if (end && !Number.isNaN(end.getTime()) && today >= start && today <= end) return today;
    return start;
  };

  const getWeekStart = (date: Date, offsetWeeks = weekOffset) => {
    const result = new Date(date);
    const dayIndex = result.getDay() === 0 ? 6 : result.getDay() - 1;
    result.setDate(result.getDate() - dayIndex + offsetWeeks * 7);
    return result;
  };

  const getSpecificDateForDay = (day: string, semesterId: string, offsetWeeks = weekOffset) => {
    const dayIndex = DAYS_OF_WEEK.indexOf(day);
    if (dayIndex < 0) return "";
    const weekStart = getWeekStart(getSemesterAnchorDate(semesterId), offsetWeeks);
  ...
  const getSectionSessionCount = (section: CourseSection) => {
    const course = store.courses.find(c => c.id === section.courseId);
    const count = Number(section.numberOfSessions || (course as any)?.numberOfLessons || 0);
    return Number.isFinite(count) && count > 0 ? Math.floor(count) : Math.max(section.schedule?.length || 0, 1);
  };

  const getSectionStartDate = (section: CourseSection) => {
    const semester = (store.semesters || []).find(s => s.id === section.semesterId);
    return String(section.openingDate || semester?.startDate || new Date().toISOString()).slice(0, 10);
  };

  const sortScheduleSlots = (slots: CourseSection["schedule"]) => [...(slots || [])].sort((a, b) => {
    ...
    const dayDiff = (DAYS_OF_WEEK.indexOf(a.dayOfWeek) + 7) % 7 - ((DAYS_OF_WEEK.indexOf(b.dayOfWeek) + 7) % 7);
    if (dayDiff !== 0) return dayDiff;
    return String(a.startTime || "").localeCompare(String(b.startTime || ""));
  });

  const getDateForScheduleSlot = (baseDate: string, slot: CourseSection["schedule"][number], cycle: number) => {
    if (slot.specificDate) return addDaysToDateString(String(slot.specificDate).slice(0, 10), cycle * 7);
    const targetDayIndex = DAYS_OF_WEEK.indexOf(slot.dayOfWeek);
    if (targetDayIndex < 0) return addDaysToDateString(baseDate, cycle * 7);
    const [year, month, day] = baseDate.split("-").map(Number);
    const start = new Date(Date.UTC(year, month - 1, day));
    const jsDay = start.getUTCDay();
    const currentDayIndex = jsDay === 0 ? 6 : jsDay - 1;
    const delta = (targetDayIndex - currentDayIndex + 7) % 7;
    return addDaysToDateString(baseDate, delta + cycle * 7);
  };

  const getSectionOccurrences = (section: CourseSection) => {
    const slots = sortScheduleSlots(section.schedule || []);
    if (slots.length === 0) return [];
    const count = getSectionSessionCount(section);
    const baseDate = getSectionStartDate(section);
    return Array.from({ length: count }, (_, index) => {
      const slot = slots[index % slots.length];
      const cycle = Math.floor(index / slots.length);
      const date = getDateForScheduleSlot(baseDate, slot, cycle);
      return {
        ...slot,
        dayOfWeek: getDayForDateString(date) || slot.dayOfWeek,
        specificDate: date,
  ...
  const getRenderSections = (): CourseSection[] => {
    ...
    if (role === "student") {
      const myRegIds = (store.courseRegistrations || [])
        .filter(r => r.studentId === currentUser.id && !["dropped", "waitlisted", "withdrawn"].includes(r.status))
        .map(r => r.sectionId);
    ...
    if (lookupType === "student" && lookupUserId) {
      const studentRegs = (store.courseRegistrations || [])
        .filter(r => r.studentId === lookupUserId && !["dropped", "waitlisted", "withdrawn"].includes(r.status))
        .map(r => r.sectionId);
  ...
  renderedSections.forEach(sec => {
    ...
    getSectionOccurrences(sec).filter(isSlotVisibleInCurrentWeek).forEach(slot => {
  ...
  const sortedScheduleList = renderedSections.flatMap(sec => {
    ...
    return getSectionOccurrences(sec).filter(isSlotVisibleInCurrentWeek).map(slot => ({
      ...slot,
      section: sec,
      courseTitle,
      teacherName
  ...
  const handleSaveSection = async (e: React.FormEvent) => {
    ...
    if (!formCourseId || !formTeacherId || !formSectionCode.trim()) {
      triggerToast("Vui lòng nhập đầy đủ: Môn học, Giảng viên và Mã lớp!");
      return;
    }
  ...
  const handleOpenCreate = () => {
    ...
    if (role === "teacher" && allowedCourses.length === 0) {
      triggerToast("Bạn cần khởi tạo ít nhất một khóa học trước khi lập lớp học phần!");
      return;
    }
  ...
  const handleDropSection = async (
    ...
    if (!sec) {
      triggerToast("Lớp học phần không hợp lệ hoặc không tìm thấy!");
      return;
    }
  ...
  const handleDeleteSection = async (id: string, code: string) => {
    ...
    try {
      ...
      onRefreshData();
      triggerToast(`Đã xóa lớp học phần ${code}!`);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto">
          ...
          {weekOffset !== 0 && (
            <button
              ...
              className="px-2.5 py-1.5 rounded-xl border border-white/10 bg-white/5 text-[10px] font-bold text-white/70 hover:text-white hover:bg-white/10 transition cursor-pointer"
            >
              Tuần hiện tại
            </button>
          )}
      ...
      {role === "admin" && (
        <div className="bg-white/3 border border-white/5 p-4 rounded-3xl grid grid-cols-1 md:grid-cols-4 gap-4 text-xs">
          ...
          <div className="flex items-end">
            {(lookupUserId || searchRoom) && (
              <button
                onClick={() => {
                  setLookupType("student");
                  setLookupUserId("");
                  setSearchRoom("");
                }}
                          ...
                          <div className="space-y-2 h-full flex flex-col justify-start">
                            {sessions.map((item, idx) => (
                              <div
                                ...
                                onClick={() => setViewingSectionDetail(item.section)}
                                className={`p-2.5 rounded-2xl border text-[11px] leading-tight space-y-1.5 transition-all shadow-md backdrop-blur-sm relative overflow-hidden group cursor-pointer ${
                                  role === "admin" ? "cursor-grab active:cursor-grabbing hover:border-indigo-400" : "hover:border-indigo-500/40"
                                } ${
                                  role === "student" 
                                ...
                                <div className="flex justify-between items-start gap-1 font-bold">
                                  ...
                                  {role === "admin" && (
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition duration-150">
                                      <button
                                        onClick={(e) => { e.stopPropagation(); handleOpenEdit(item.section); }}
                                      ...
                                      <button
                                        onClick={(e) => { e.stopPropagation(); handleDeleteSection(item.section.id, item.section.sectionCode); }}
      ...
      {(viewMode === "list" || viewMode === "grid") && (
        <div className={`${viewMode === "grid" ? "block md:hidden" : "block"} space-y-4`}>
          {sortedScheduleList.map((slot, idx) => (
            <div
              key={idx}
              onClick={() => setViewingSectionDetail(slot.section)}
              className="bg-white/3 border border-white/10 rounded-3xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition duration-150 hover:bg-white/5 cursor-pointer"
            >
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-white/50 text-[11px]">
                    ...
                    {slot.specificDate ? (
                      ...
                      <span className="flex items-center gap-1 font-sans text-cyan-400 font-semibold">
                        <Calendar className="h-3.5 w-3.5 shrink-0" />
                        Ngày: {getSpecificDateForDay(slot.dayOfWeek, activeSemesterId)}
                      </span>
                    )}
                    {role !== "teacher" && (
                      <span className="flex items-center gap-1">
              ...
              {(role === "admin" || (role === "teacher" && slot.section.teacherId === currentUser.id)) && (
                <div className="flex gap-2 self-end sm:self-auto items-center">
                  ...
                  {role === "admin" && (
                    <>
                      <button
                        onClick={(e) => { e.stopPropagation(); handleOpenEdit(slot.section); }}
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); handleDeleteSection(slot.section.id, slot.section.sectionCode); }}
      ...
      {viewingSectionDetail && (() => {
        const section = viewingSectionDetail;
        const course = store.courses.find(c => c.id === section.courseId);
        const semester = store.semesters.find(s => s.id === section.semesterId);
          ...
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200 text-white font-sans">
              ...
              <div className="flex justify-between items-center bg-white/3 px-6 py-4 border-b border-white/5">
                ...
                <button
                  onClick={() => setViewingSectionDetail(null)}
              ...
              <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto pr-2 scrollbar-thin">
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="grid grid-cols-1 gap-2.5">
                    {section.schedule.map((slot, sIdx) => (

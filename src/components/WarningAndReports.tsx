...
export default function WarningAndReports({ 
  ...
  const handleCreateWarningSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (["low_attendance", "exam_ban", "overdue_assignment"].includes(selectedWarningType) && selectedCourseId) {
        payload.courseId = selectedCourseId;
      }
      await api.createWarning(payload);
  ...
  const courseSections = store.courseSections || [];
  const courseRegistrations = store.courseRegistrations || [];
  ...
  courses.forEach(c => {
    ...
    const pastSessions = courseSessions.filter(s => new Date(s.date).getTime() <= Date.now());
    ...
    courseEnroll.forEach(enroll => {
      const records = attendanceRecords.filter(r => 
        ...
        pastSessions.some(cs => cs.id === r.sessionId)
      );

      const presentCount = records.filter(r => r.status === "present" || r.status === "late" || r.status === "excused").length;
      const pct = Math.round((presentCount / pastSessions.length) * 100);
      if (pct < 80) {
        const studentUser = users.find(u => u.id === enroll.studentId);
        // Find registration to get the section code
        const studentReg = courseRegistrations.find(cr => 
          cr.studentId === enroll.studentId && 
          cr.status === "registered" &&
          courseSections.some(cs => cs.id === cr.sectionId && cs.courseId === c.id)
        );
        const sectionObj = studentReg ? courseSections.find(cs => cs.id === studentReg.sectionId) : null;
        const sectionCode = sectionObj ? sectionObj.sectionCode : "";
        const courseDisplay = sectionCode ? `${c.title} (${sectionCode})` : c.title;
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/95 overflow-y-auto p-4 pt-6 md:pt-10 border border-white/10 backdrop-blur-xl animate-fade-in flex flex-col">
          <div className="space-y-6">
            ...
            <form onSubmit={handleCreateWarningSubmit} className="space-y-4 text-xs font-sans">
              ...
              {["low_attendance", "exam_ban", "overdue_assignment"].includes(selectedWarningType) && (
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-white/70 block">Môn học liên quan <span className="text-red-400">*</span></label>
                  <select
            ...
            {formattedWarnings.map(w => (
              <div 
                ...
                className={`p-4 border rounded-2xl relative overflow-hidden flex flex-col justify-between transition ${
                  w.isResolved 
                    ? "bg-emerald-500/10 dark:bg-emerald-950/40 hover:border-emerald-500/50 border-emerald-500/20 dark:border-emerald-500/30 text-emerald-800 dark:text-white/70" 
                    : "bg-red-500/10 dark:bg-red-950/40 hover:border-red-500/50 border-red-500/20 dark:border-red-500/30 text-red-800 dark:text-white"
                }`}
              >
                <div className="space-y-1">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-[9px] uppercase tracking-wider font-extrabold px-2 py-0.5 rounded bg-white/5 text-slate-300 font-mono flex items-center gap-1.5 flex-wrap">
                      <span>{warningTypeLabel(w.type)}</span>
                      {w.courseId && (() => {
                        const course = store.courses.find(c => c.id === w.courseId);
                        return course ? (
                          <span className="px-1.5 py-0.2 rounded bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 font-sans font-bold">
                            {course.title}
                          </span>
                        ) : null;
                      })()}
                  ...
                  <p className="text-[11.5px] leading-relaxed pt-1.5 text-slate-800 dark:text-white/80">{w.message}</p>
                </div>
